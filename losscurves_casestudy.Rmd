...
The Stan sample contains no divergent transitions, so that is a good
start.
...
The $\hat{R}$ values for the parameter appear to be in or around 1, so
that is encouraging.

It is also worth checking the size of `n_eff` for each of the
variables:
...
The package `bayesplot` provides some simple diagnostic plots for
fits, so we look at those.

```{r model1_bayesplot_rhat, echo=TRUE}
