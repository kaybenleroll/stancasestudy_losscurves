...
time. To ensure this, we filter out all data timestamped after 1997 and
use the remaining data as our modelling dataset.
...
# Initial Stan Model - Single Line-of-Business, Single Insurer
...
We scale the losses by the total premium received for that cohort, the
total losses is then given by

$$
\text{Total Loss}(t) = \text{Premium} \; \text{x} \;
  \text{Final Loss Ratio} \; \text{x} \; \text{GF}(t)
$$
...
\begin{eqnarray}
\mu(Y, t) &=& \text{Premium}(Y) \; \text{x} \; \text{LR}(Y)
  \; \text{x} \; \text{GF}(t)  \\

\sigma_Y &=& \text{Premium}(Y) \; \text{x} \; \sigma \\

\text{LR}_Y &\sim& \text{Lognormal}(\mu_{\text{LR}}, \sigma_{\text{LR}}) \\

\mu_{\text{LR}} &\sim& \textbf{Normal}(0, 0.5)
\end{eqnarray}

All other parameters in the model, ($\sigma_{\text{LR}}$, $\omega$,
$\theta$, $\sigma$), have lognormal priors to constrain them to be
positive. The hyper-parameters for the lognormal priors are relatively
weak.
...
weibull_tbl <- tibble(label = 'Weibull'
                     ,t = t_seq
                     ,value = weibull_func(t_seq, 1.5, 2.2)
                      )

loglogistic_tbl <- tibble(label = 'Log-logistic'
                         ,t = t_seq
...
fittedweibull_tbl <- tibble(label = 'Weibull (fitted)'
                           ,t = t_seq
...
ggplot(plot_tbl) +
    geom_line(aes(x = t, y = value, colour = label)) +
    xlab(expression(t)) +
    ylab(expression("Growth Factor for Functional Forms"))
```
...
```{r model_sislob_convergenceplot, echo=TRUE}
...
It is also worth checking the size of `n_eff` for each of the variables:
...
The packages `bayesplot` provides some simple diagnostic plots for fits, so we look at those.
...
model_sislob_stanfit %>%
    ...
    mcmc_neff(.) + yaxis_text()
```

We also look for the autocorrelation in various parameters:
...
As we are using the NUTS algorithm to take our sample from the posterior, we may use some specific diagnostics to check how effective it was.
...
And we check the traces for convergence too, checking to see if they mix.
...
Having convinced ourselves that the samples have converged, we proceed to checking the quality of the fit. We check the 50\% credibility intervals of the parameters
...
model_sislob_ll_list <- create_stanfit(model_sislob_stanmodel, usedata_tbl, model_id = 0)
...
```{r model_sislob_ll_bayesplot_rhat, echo=TRUE}
...
```{r model_sislob_ll_bayesplot_neff, echo=TRUE}
model_sislob_ll_stanfit %>%
    neff_ratio(pars = stanmodel_pars) %>%
    mcmc_neff(.) + yaxis_text()
```
...
```{r model_sislob_ll_bayesplot_acf_omegatheta, echo=TRUE}
model_sislob_ll_stanfit %>%
    extract(inc_warmup = FALSE, permuted = FALSE) %>%
    mcmc_acf(pars = c('omega','theta'))
```

```{r model_sislob_ll_bayesplot_acf_LR, echo=TRUE, fig.height=14}
model_sislob_ll_stanfit %>%
    extract(inc_warmup = FALSE, permuted = FALSE) %>%
    mcmc_acf(pars = c('mu_LR','LR[3]'))
```

Finally, we check the energy diagnostic plots to check they are similar.

```{r model_sislob_ll_energy_diagnostic, echo=TRUE}
model_sislob_ll_stanfit %>%
    nuts_params %>%
    mcmc_nuts_energy(binwidth = 1)
```
...
```{r model_sislob_ll_bayesplot_traces, echo=TRUE}
model_sislob_ll_stanfit %>%
    as.matrix %>%
    mcmc_trace(regex_pars = c('theta','omega','LR\\[','mu_LR','sd_LR','loss_sd'))
```
...
```{r model_sislob_grcode353_wb_bayesplot_rhat, echo=TRUE}
...
```{r model_sislob_grcode353_wb_bayesplot_neff, echo=TRUE}
model_sislob_grcode353_wb_stanfit %>%
    neff_ratio(pars = stanmodel_pars) %>%
    mcmc_neff(.) + yaxis_text()
```
...
```{r model_sislob_grcode353_wb_bayesplot_acf_omegatheta, echo=TRUE}
model_sislob_grcode353_wb_stanfit %>%
    extract(inc_warmup = FALSE, permuted = FALSE) %>%
    mcmc_acf(pars = c('omega','theta'))
```

```{r model_sislob_grcode353_wb_bayesplot_acf_LR, echo=TRUE, fig.height=14}
model_sislob_grcode353_wb_stanfit %>%
    extract(inc_warmup = FALSE, permuted = FALSE) %>%
    mcmc_acf(pars = c('mu_LR','LR[3]'))
```
...
```{r model_sislob_grcode353_wb_energy_diagnostic, echo=TRUE}
model_sislob_grcode353_wb_stanfit %>%
    nuts_params %>%
    mcmc_nuts_energy(binwidth = 1)
```
...
```{r model_sislob_grcode353_wb_bayesplot_traces, echo=TRUE}
model_sislob_grcode353_wb_stanfit %>%
    as.matrix %>%
    mcmc_trace(regex_pars = c('theta','omega','LR\\[','mu_LR','sd_LR','loss_sd'))
```
...
```{r model_sislob_grcode353_ll_bayesplot_rhat, echo=TRUE}
model_sislob_grcode353_ll_stanfit %>%
    rhat(pars = stanmodel_pars) %>%
    mcmc_rhat(.) + yaxis_text()
```
...
```{r model_sislob_grcode353_ll_bayesplot_neff, echo=TRUE}
model_sislob_grcode353_ll_stanfit %>%
    neff_ratio(pars = stanmodel_pars) %>%
    mcmc_neff(.) + yaxis_text()
```
...
```{r model_sislob_grcode353_ll_bayesplot_acf_omegatheta, echo=TRUE}
model_sislob_grcode353_ll_stanfit %>%
    extract(inc_warmup = FALSE, permuted = FALSE) %>%
    mcmc_acf(pars = c('omega','theta'))
```

```{r model_sislob_grcode353_ll_bayesplot_acf_LR, echo=TRUE, fig.height=14}
model_sislob_grcode353_ll_stanfit %>%
    extract(inc_warmup = FALSE, permuted = FALSE) %>%
    mcmc_acf(pars = c('mu_LR','LR[3]'))
```
...
```{r model_sislob_grcode353_ll_energy_diagnostic, echo=TRUE}
model_sislob_grcode353_ll_stanfit %>%
    nuts_params %>%
    mcmc_nuts_energy(binwidth = 1)
```
...
```{r model_sislob_grcode353_ll_bayesplot_traces, echo=TRUE}
model_sislob_grcode353_ll_stanfit %>%
    as.matrix %>%
    mcmc_trace(regex_pars = c('theta','omega','LR\\[','mu_LR','sd_LR','loss_sd'))
```
...
```{r model_sislob_grcode353_ll_bayesplot_parameters, echo=TRUE}
model_sislob_grcode353_ll_stanfit %>%
    extract(inc_warmup = FALSE, permuted = FALSE) %>%
    mcmc_intervals(regex_pars = c('omega','theta','LR\\[', 'mu_LR','sd_LR','loss_sd'))
```
