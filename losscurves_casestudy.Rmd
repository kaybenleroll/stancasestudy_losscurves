...
We now repeat the previous exercise of checking that the sample has converged
through the use of the various diagnostic tools employed before.

## Diagnostic Plots

We first check $\hat{R}$.
...
We then check the related quantity of effective sample size, $n_{\text{eff}}$:
...
The above plots do not reveal anything of concern so we look at the ACF plots
for the parameters, as before.
...
We check that the energy diagnostic plots show us similar distributions.
...
Finally we look at the parameter traceplots to look for signs that the chains
have not mixed.
...
The chains appear to have mixed, none of the diagnostics have alerted us to
anything and there are no divergent transitions, so we can proceed with
checking the model.
