...
cohort_dt <- unique(snapshot_dt[, .(acc_year, premium)])
cohort_dt[, cohort_id := .I]

usedata_dt <- snapshot_dt

stan_data_lst <- list(growthmodel_id = 1   # Use weibull rather than loglogistic
                     ,n_data         = usedata_dt[, .N]
                     ,n_time         = usedata_dt[, length(unique(dev_lag))]
                     ,n_cohort       = cohort_dt[, .N]
                     ,cohort_id      = match(usedata_dt$acc_year, cohort_dt$acc_year)
                     ,cohort_maxtime = usedata_dt[, .(maxtime = max(dev_lag)), by = acc_year][order(acc_year)]$maxtime
                     ,t_value        = usedata_dt[, sort(unique(dev_lag))]
                     ,t_idx          = usedata_dt[, match(dev_lag, sort(unique(dev_lag)))]
