...
claimdata_tbl <- claimdata_tbl %>%
    ...
    select(grcode, lob, acc_year, dev_year, dev_lag, premium, cum_loss, loss_ratio)
```
...
```{r model_sislob_stanmodel, cache=FALSE, warning=TRUE, message=FALSE}
...
```{r model_sislob_bayesplot_traces, echo=TRUE}
model_sislob_stanfit %>%
    as.matrix %>%
